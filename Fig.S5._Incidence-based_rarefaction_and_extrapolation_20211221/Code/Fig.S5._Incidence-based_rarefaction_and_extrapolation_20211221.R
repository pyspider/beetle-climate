...
loss.out.inci <- iNEXT3D(list.loss.inci_V2, diversity = 'TD', q = c(0,1,2), datatype = "incidence_raw")
save(loss.out.inci,file="p.loss.inci_OTU_20211220")
...
library(reshape) #reshape v0.8.8
library(patchwork) #patchwork v1.0.0
library(iNEXT.3D) #iNEXT.3D v0.0.1
library(phytools) #phytools v0.7-80
library(phangorn) #phangorn v2.5.5
library(rlist) #rlist v0.4.6.1
library(phyclust) #phyclust v0.1-30
...
# Hill numbers (q):  0 = sp richness, 1 = Shannon, 2 = inverse Simpson
pd.unifrac.inci <- iNEXT3D(unifrac.inci.loss.set, nT=nt, diversity = 'PD', PDtype = "PD", PDtree = iqtree.treepl, q=c(0,1,2), datatype="incidence_raw")
save(pd.unifrac.inci,file="pd.inci_denovo_treepl_pd_20211218")
...
# Hill numbers (q):  0 = sp richness, 1 = Shannon, 2 = inverse Simpson
pd.mito.inci <- iNEXT3D(mito.inci.loss.set, nT=nt, diversity = 'PD', PDtype = 'PD', PDtree = mito.treepl, q=c(0,1,2), datatype="incidence_raw")
save(pd.mito.inci,file="pd.inci_mito_treepl_pd_20211218") 

##################
#plot the results of incidence-based rarefaction and extrapolation 
##################

load("Data/p.loss.inci_OTU_20211220")
otu.inci<-loss.out.inci$iNextEst$size_based
df.point <- otu.inci[which(otu.inci$Order.q=="0"),]
df.point <- df.point[which(df.point$Method=="Observed"),]
df.line <- otu.inci[which(otu.inci$Order.q=="0"),]
df.line.Extrapolation <- otu.inci[which(otu.inci$Order.q=="0"&otu.inci$Method=="Extrapolation"),]
cols <- c("#1d953f","#1d953f","#1d953f","black","black","black","#8a5656","#8a5656","#8a5656","#D55E00","#D55E00")

p0.otu.inci<-ggplot(filter(otu.inci,otu.inci$Method!="Extrapolation"), aes(x=nt, y=qD, colour=Assemblage))  +
...
df.point <- otu.inci[which(otu.inci$Order.q=="1"),]
df.point <- df.point[which(df.point$Method=="Observed"),]
df.line <- otu.inci[which(otu.inci$Order.q=="1"),]
df.line.Extrapolation <- otu.inci[which(otu.inci$Order.q=="1"&otu.inci$Method=="Extrapolation"),]

p1.otu.inci<-ggplot(filter(otu.inci,otu.inci$Method!="Extrapolation"), aes(x=nt, y=qD, colour=Assemblage))  +
  geom_point(aes(shape=Assemblage), size=5, data=df.point) +
  geom_line(aes(linetype=c("1")), lwd=1.5,data =filter(df.line,df.line$Method!="Extrapolation")) +
  geom_line(aes(linetype=c("2")), lwd=1.5,data =df.line.Extrapolation) +
  geom_ribbon(aes(ymin=qD.LCL, ymax=qD.UCL,
...
df.point <- otu.inci[which(otu.inci$Order.q=="2"),]
df.point <- df.point[which(df.point$Method=="Observed"),]
df.line <- otu.inci[which(otu.inci$Order.q=="2"),]
df.line.Extrapolation <- otu.inci[which(otu.inci$Order.q=="2"&otu.inci$Method=="Extrapolation"),]

p2.otu.inci<-ggplot(filter(otu.inci,otu.inci$Method!="Extrapolation"), aes(x=nt, y=qD, colour=Assemblage))  +
  geom_point(aes(shape=Assemblage), size=5, data=df.point) +
  geom_line(aes(linetype=c("1")), lwd=1.5,data =filter(df.line,df.line$Method!="Extrapolation")) +
  geom_line(aes(linetype=c("2")), lwd=1.5,data =df.line.Extrapolation) +
  geom_ribbon(aes(ymin=qD.LCL, ymax=qD.UCL,
...
load("Data/pd.inci_denovo_treepl_pd_20211218")
pd.inci.denovo<-pd.unifrac.inci$PDiNextEst$size_based
df.point <- pd.inci.denovo[which(pd.inci.denovo$Order.q=="0"),]
df.point <- df.point[which(df.point$Method=="Observed"),]
df.line <- pd.inci.denovo[which(pd.inci.denovo$Order.q=="0"),]
df.line.Extrapolation <- pd.inci.denovo[which(pd.inci.denovo$Order.q=="0"&pd.inci.denovo$Method=="Extrapolation"),]
cols <- c("#1d953f","#1d953f","#1d953f","black","black","black","#8a5656","#8a5656","#8a5656","#D55E00","#D55E00")

p0.denovo.inci<-ggplot(filter(pd.inci.denovo,pd.inci.denovo$Method!="Extrapolation"), aes(x=nt, y=qPD, colour=Assemblage))  +
  geom_point(aes(shape=Assemblage), size=5, data=df.point) +
  geom_line(aes(linetype=c("1")), lwd=1.5,data =filter(df.line,df.line$Method!="Extrapolation")) +
  geom_line(aes(linetype=c("2")), lwd=1.5,data =df.line.Extrapolation) +
...
df.point <- pd.inci.denovo[which(pd.inci.denovo$Order.q=="1"),]
df.point <- df.point[which(df.point$Method=="Observed"),]
df.line <- pd.inci.denovo[which(pd.inci.denovo$Order.q=="1"),]
df.line.Extrapolation <- pd.inci.denovo[which(pd.inci.denovo$Order.q=="1"&pd.inci.denovo$Method=="Extrapolation"),]
cols <- c("#1d953f","#1d953f","#1d953f","black","black","black","#8a5656","#8a5656","#8a5656","#D55E00","#D55E00")

p1.denovo.inci<-ggplot(filter(pd.inci.denovo,pd.inci.denovo$Method!="Extrapolation"), aes(x=nt, y=qPD, colour=Assemblage))  +
  geom_point(aes(shape=Assemblage), size=5, data=df.point) +
  geom_line(aes(linetype=c("1")), lwd=1.5,data =filter(df.line,df.line$Method!="Extrapolation")) +
  geom_line(aes(linetype=c("2")), lwd=1.5,data =df.line.Extrapolation) +
  geom_ribbon(aes(ymin=qPD.LCL, ymax=qPD.UCL,
...
df.point <- pd.inci.denovo[which(pd.inci.denovo$Order.q=="2"),]
df.point <- df.point[which(df.point$Method=="Observed"),]
df.line <- pd.inci.denovo[which(pd.inci.denovo$Order.q=="2"),]
df.line.Extrapolation <- pd.inci.denovo[which(pd.inci.denovo$Order.q=="2"&pd.inci.denovo$Method=="Extrapolation"),]
cols <- c("#1d953f","#1d953f","#1d953f","black","black","black","#8a5656","#8a5656","#8a5656","#D55E00","#D55E00")

p2.denovo.inci<-ggplot(filter(pd.inci.denovo,pd.inci.denovo$Method!="Extrapolation"), aes(x=nt, y=qPD, colour=Assemblage))  +
  geom_point(aes(shape=Assemblage), size=5, data=df.point) +
  geom_line(aes(linetype=c("1")), lwd=1.5,data =filter(df.line,df.line$Method!="Extrapolation")) +
  geom_line(aes(linetype=c("2")), lwd=1.5,data =df.line.Extrapolation) +
  geom_ribbon(aes(ymin=qPD.LCL, ymax=qPD.UCL,
...
load("Data/pd.inci_mito_treepl_pd_20211218")
pd.inci.mito<-pd.mito.inci$PDiNextEst$size_based
df.point <- pd.inci.mito[which(pd.inci.mito$Order.q=="0"),]
df.point <- df.point[which(df.point$Method=="Observed"),]
df.line <- pd.inci.mito[which(pd.inci.mito$Order.q=="0"),]
df.line.Extrapolation <- pd.inci.mito[which(pd.inci.mito$Order.q=="0"&pd.inci.mito$Method=="Extrapolation"),]
cols <- c("#1d953f","#1d953f","#1d953f","black","black","black","#8a5656","#8a5656","#8a5656","#D55E00","#D55E00")

p0.mito.inci<-ggplot(filter(pd.inci.mito,pd.inci.mito$Method!="Extrapolation"), aes(x=nt, y=qPD, colour=Assemblage))  +
  geom_point(aes(shape=Assemblage), size=5, data=df.point) +
  geom_line(aes(linetype=c("1")), lwd=1.5,data =filter(df.line,df.line$Method!="Extrapolation")) +
  geom_line(aes(linetype=c("2")), lwd=1.5,data =df.line.Extrapolation) +
  geom_ribbon(aes(ymin=qPD.LCL, ymax=qPD.UCL,
...
df.point <- pd.inci.mito[which(pd.inci.mito$Order.q=="1"),]
df.point <- df.point[which(df.point$Method=="Observed"),]
df.line <- pd.inci.mito[which(pd.inci.mito$Order.q=="1"),]
df.line.Extrapolation <- pd.inci.mito[which(pd.inci.mito$Order.q=="1"&pd.inci.mito$Method=="Extrapolation"),]
cols <- c("#1d953f","#1d953f","#1d953f","black","black","black","#8a5656","#8a5656","#8a5656","#D55E00","#D55E00")

p1.mito.inci<-ggplot(filter(pd.inci.mito,pd.inci.mito$Method!="Extrapolation"), aes(x=nt, y=qPD, colour=Assemblage))  +
  geom_point(aes(shape=Assemblage), size=5, data=df.point) +
  geom_line(aes(linetype=c("1")), lwd=1.5,data =filter(df.line,df.line$Method!="Extrapolation")) +
  geom_line(aes(linetype=c("2")), lwd=1.5,data =df.line.Extrapolation) +
  geom_ribbon(aes(ymin=qPD.LCL, ymax=qPD.UCL,
...
df.point <- pd.inci.mito[which(pd.inci.mito$Order.q=="2"),]
df.point <- df.point[which(df.point$Method=="Observed"),]
df.line <- pd.inci.mito[which(pd.inci.mito$Order.q=="2"),]
df.line.Extrapolation <- pd.inci.mito[which(pd.inci.mito$Order.q=="2"&pd.inci.mito$Method=="Extrapolation"),]
cols <- c("#1d953f","#1d953f","#1d953f","black","black","black","#8a5656","#8a5656","#8a5656","#D55E00","#D55E00")

p2.mito.inci<-ggplot(filter(pd.inci.mito,pd.inci.mito$Method!="Extrapolation"), aes(x=nt, y=qPD, colour=Assemblage))  +
  geom_point(aes(shape=Assemblage), size=5, data=df.point) +
  geom_line(aes(linetype=c("1")), lwd=1.5,data =filter(df.line,df.line$Method!="Extrapolation")) +
  geom_line(aes(linetype=c("2")), lwd=1.5,data =df.line.Extrapolation) +
  geom_ribbon(aes(ymin=qPD.LCL, ymax=qPD.UCL,
...
dev.new()
pdf("Fig.S5.inext3d_inci_20211220.pdf",width=17,height=16)
p.otu.inci/p.denovo.inci/p.mito.inci 
dev.off()